...
```{r Load data}
  ...
  s_df = read.csv('../Data/002_stroop.csv')
  d_df = read.csv('../Data/002_decision.csv')
```
...
```{r decision analysis}
  ...
  d_df_clean$lag_FB = as.factor(lag(d_df_clean$FB))
...
```{r Group Analysis}
  s_df = read.csv('../Data/stroop_all_subj.csv')
  d_df = read.csv('../Data/decision_all_subj.csv')
```
...
```{r decision analysis}
  ...
  decision_rs = glmer(Switch ~ lag_FB * lag_transition +(lag_FB*lag_transition | id), family = binomial, data = d_df_clean)
  summary(decision_rs)
```
