...
```{r stroop analysis}
  ...
  stroop_df_clean$rt_logZ = scale(log(stroop_df_clean$rt))
  stroop_rs = lm(rt_logZ ~ congruent * type, data = stroop_df_clean)
  summary(stroop_rs)
  #back out of Z/log transform to interpret
  log_rt_mean = mean(log(stroop_df_clean$rt))
  log_rt_sd = sd(log(stroop_df_clean$rt))
